...
- このスキルを起動したら、変更要件ヒアリングに入る前に必ず以下2問のみをこの順番で質問すること。
  - 質問1: 今回の変更で解決すべき業務課題は何ですか（課題名、現状の問題、業務影響、解決状態）
  - 質問2: 外部システム連携はありますか（ある / ない）
- 質問1の回答をもとに、変更対象の業務課題を `RQ-BK-*` として確定するまで、変更要件の質問を開始してはならない。
- 上記2問の回答が確定するまで、変更要件の質問を開始してはならない。
- 分岐ルール:
  - 外部システム連携が「ある」場合は `isdd-external-precheck` を先に実行し、完了確認後に変更要件ヒアリングへ進む。
  - 外部システム連携が「ない」場合は、そのまま変更要件ヒアリングへ進む。
- 停止条件:
  - 回答が未確定、`RQ-BK-*` の採番が未完了、または `isdd-external-precheck` の完了未確認の場合は、変更要件定義の本編を開始せず不足情報の確認のみを行うこと。
...
- 開始ゲートの2問、`RQ-BK-*` 採番、先行スキル完了確認が終わるまで、変更要件ヒアリング本編に入らない
