...
- 備品一覧（一般ユーザー向け）：資産管理番号、備品名、状態（貸出可能/貸出中）
- 備品一覧（管理者向け追加表示）：上記に加え、貸出中の備品には貸出先ユーザー名・貸出日を表示
...
| RQ-TS-GENERAL-USER-VIEW | 一般ユーザーの閲覧・操作制限確認 | 一般ユーザーでログイン済み | 備品一覧画面を閲覧し、管理者用ボタン（貸出・返却・編集・削除・新規登録）が表示されないことを確認する | 資産管理番号・備品名・状態（貸出可能/貸出中）のみ表示され、操作ボタンは表示されない | RQ-BK-UNKNOWN-HOLDER, RQ-BK-MULTI-LEDGER |
