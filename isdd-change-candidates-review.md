...
上記前提のもとで、設計未マッピング要件の一覧化と逐次ヒアリングを標準運用とする。

#### 設計未マッピング要件の一覧化と逐次ヒアリング
...
要件変更が必要になった場合は、4.4で定義した方式に従い、未マッピング一覧をもとに要件定義を1件ずつヒアリングで更新する。変更要件起票や起票テンプレート運用は行わない。
...
- 実装後整合レポートの参照を必須化する。
- 要件修正が必要な場合は、4.4方式で要件更新が完了するまで完了としない。
...
変更後:
- 実装後整合で戻された差分を4.4方式の未マッピング一覧として受け取る。
- 一覧の各要件に対するユーザーヒアリング完了後、更新済み要件IDを受け取り再設計する。
- 変更要件定義、変更設計は新規要求の追加時のみ利用し、同一要求内の修正では利用しない。

### 5.4 共通部品化する範囲

| 共通部品 | 既存/新規/変更 | 役割 | 具体変更内容/具体部品内容 | 主な利用スキル |
|---|---|---|---|---|
| 構造抽出部品 | 既存を変更 | モジュール、クラス、関数、依存関係の抽出 | 既存の構造解析処理に、実装差分起点の抽出対象指定と抽出結果の安定ソートを追加する。初回逆引きと実装後逆引きで同一形式を返すようにする。 | reverse-engineering, post-implementation-review |
| RQ-DS突合部品 | 既存を変更 | 要件IDと設計IDの未対応、重複、不整合の判定 | 既存の突合ロジックへ、未マッピング一覧を要件単位で返す出力モードを追加する。重複、不整合、未対応を別カテゴリで返し、4.4のヒアリングに直接渡せる形式へ変更する。 | design系, traceable-coding, post-implementation-review |
| 差分要約部品 | 新規 | 実装差分の仕様影響点抽出 | Git差分から、影響した要件候補、設計候補、依存関係変化を抽出する部品。変更ファイル、関数、参照IDをまとめて返す。 | post-implementation-review |
| 未マッピング一覧生成部品 | 新規 | 4.4方式で使う要件単位の差分一覧生成 | 突合結果と差分要約結果を統合し、要件ごとに「現状」「問題点」「ヒアリング質問」「更新候補」を1レコード化して一覧化する部品。 | design系, post-implementation-review |
| 要件ヒアリング連携部品 | 新規 | 一覧の要件を1件ずつヒアリング順へ変換 | 未マッピング一覧を質問順へ整形し、1問1件の進行制御を行う部品。回答結果を要件更新差分として保持し、次質問へ連鎖させる。 | requirements系, change-req系 |
| ID再採番伝播部品 | 新規 | 更新後の要件IDを設計IDへ再伝播 | 要件更新後のID差分を読み取り、設計書内の対応IDを再採番して置換する部品。旧IDと新IDの対応表を出力し、再設計時の整合確認に利用する。 | design系, change-design系 |
...
5. 未マッピング一覧を生成する。
6. 4.4方式に従い、一覧の要件を1件ずつユーザーヒアリングして要件定義を更新する。
...
- 論点3と論点4の方式定義を統一し、要件更新フローを同一手順で接続した。
...
- 論点3は4.1〜4.3を削除し、4.4の確定運用へ更新した。
- 論点4は4.4と接続し、起票なしの要件ヒアリング更新とID伝播を反映した。
- 論点4の共通部品表へ既存、新規、変更の区分と具体変更内容を追加した。
- 論点5は変更点説明のみの構成に統一した。
